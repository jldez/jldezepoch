...
def gammaCondense(omega,E): 
	return omega/(e*E)*sqrt(mReduced*me*bandGap)
...
def Wcondense(omega,E):
	return 2.0*omega/(9.0*pi*densiteAtome)*(omega*mReduced*me/(hbar*sqrt(grandGamma(gammaCondense(omega,E)))))**1.5*\
...
def WcondenseTunnel(omega,E):
	return 2.0/(9.0*pi**2.0*densiteAtome)*bandGap/hbar*(mReduced*me*bandGap/(hbar**2.0))**1.5*\
			(e*hbar*E/((mReduced*me)**0.5*bandGap**1.5))**2.5*\
			np.exp(-((pi/2.0)*((mReduced*me)**0.5*bandGap**1.5)/(e*hbar*E)*(1.0+(mReduced*me*omega**2.0*bandGap)/(8.0*e**2.0*E**2.0))))

def sigma(omega):
	K = np.ceil(bandGap/(hbar*omega))
	return (2.0*omega/(9.0*pi))*(mReduced*me*omega/hbar)**1.5*\
			Phi(sqrt(2.0*(K-bandGap/(hbar*omega))),m)*np.exp(2.0*K)*\
			((e**2.0)/(8.0*mReduced*me*omega**2.0*bandGap*eps0*c))**K
...
Eepoch = omega/(e*gammaEpoch)*(mReduced*me*bandGap)**0.5
